...
export class CepComponent extends AbstractTableComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    fromEvent(this.txtCodigo.nativeElement, 'keyup').pipe(
      debounceTime(500)
    ).subscribe((x) => {
      this.myTable.draw();
    });

    fromEvent(this.txtLogradouro.nativeElement, 'keyup').pipe(
      debounceTime(500)
    ).subscribe((x) => {
      this.myTable.draw();
    });

    fromEvent(this.txtBairro.nativeElement, 'keyup').pipe(
      debounceTime(500)
    ).subscribe((x) => {
      this.myTable.draw();
    });

    fromEvent(this.txtMunicipio.nativeElement, 'keyup').pipe(
      debounceTime(500)
    ).subscribe((x) => {
      this.myTable.draw();
    });
